...
class Solution79 {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                if (word.charAt(0) == board[i][j]) {
                    used[i][j] = true;
                    backtrack(board, word, 0, used, i, j);
                    used[i][j] = false;
                }
            }
        }
    ...
    private void backtrack(char[][] board, String word, int index, boolean[][] used, int x, int y) {
            ...
            for (int k = 0; k < dir.length; k += 2) {
                ...
                if (newX >= 0 && newX < board.length && newY >= 0 && newY < board[0].length) {
                    if (!used[newX][newY]) {
                        ...
                        backtrack(board, word, index + 1, used, newX, newY);
